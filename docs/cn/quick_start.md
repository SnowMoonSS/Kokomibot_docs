...
或者
@机器人 /kkm bind asia SangonomiyaKokomi_ 
或者
@机器人 /kkm 绑定 亚服 SangonomiysKokomi_ 
```
...
1. 绑定后发送指令 `kkm me bind` 可以查看当前账户的默认设置
2. 切换语言：如果想要切换为English，可以通过指令 `kkm lang en` 来切换，**但请注意不同语言指令不同**
3. 关闭评分：如果您不喜欢看到PR评分，可以通过指令 `kkm pr off` 来关闭显示。把off替换成on则可以恢复
4. Recent/Recents：**❗ 绑定账户默认不会启用该功能**，如未启用可以发送指令 `kkm recent/recents on` 来启用recent或者recents功能，但可能会有数量限制
